...
@registry.register_task('evotune')
class MaskedLanguageModelingFineTune(Dataset):
    """Creates the Masked Language Modeling Fine Tune Dataset
    Args:
        data_path (Union[str, Path]): Path to tape data root.
        split (str): One of ['train', 'valid', 'holdout'], specifies which data file to load.
        in_memory (bool, optional): Whether to load the full dataset into memory.
    ...
    def __init__(self,
        ...
        data_path = Path(data_path)
        data_file = f'finetune_{split}.fasta'

        # In memory should probably be defaulted to true here
        self.data = dataset_factory(data_path / data_file, in_memory)
    ...
    def __getitem__(self, index):
        ...
        return masked_token_ids, input_mask, labels, None, None
